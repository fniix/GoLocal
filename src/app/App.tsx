...
export default function App() {
  ...
  useEffect(() => {
    const hasActiveBooking =
      !!activeOrderId || ['driver-matching', 'live-tracking', 'payment', 'rating'].includes(currentScreen);
  ...
  useEffect(() => {
    ...
    const unsubscribe = listenOrderById(activeOrderId, (order) => {
      if (!order) return;
      // Only push to payment from active trip screens — not after user opened payment/rating
      if (
        order.status === 'completed' &&
        !['payment', 'rating', 'ride-history'].includes(currentScreen)
      ) {
